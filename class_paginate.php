      ...
      public function paginate()
      {
          ...
          if ($this->num_pages > 1) {
              if ($this->current_page != 1 && $this->items_total >= $this->default_ipp) {
                  if ($this->path) {
					  $this->retdata = "<a class=\"item\" href=\"".$this->path.$prev_page."\"><i class=\"icon left arrow\"></i></a>";
                  } else {
              ...
              for ($i = 1; $i <= $this->num_pages; $i++) {
                  ...
                  if ($i == 1 or $i == $this->num_pages or in_array($i, $this->range)) {
                      if ($i == $this->current_page) {
                          ...
                          if ($this->path) {
							  $this->retdata .= "<a class=\"item\" title=\"Go To $i of $this->num_pages\" href=\"".$this->path.$i."/\">$i</a>";
                          } else {
              ...
              if ($this->current_page != $this->num_pages && $this->items_total >= $this->default_ipp) {
                  if ($this->path) {
					  $this->retdata .= "<a class=\"item\" href=\"".$this->path.$next_page."\"><i class=\"icon right arrow\"></i></a>";
                  } else {
              ...
              for ($i = 1; $i <= $this->num_pages; $i++) {
                  if ($i == $this->current_page) {
					  ...
					  if ($this->path) {
						  $this->retdata .= "<a class=\"item\" href=\"".$this->path . $i."/".$this->items_per_page."/\">$i</a>";
					  } else {
